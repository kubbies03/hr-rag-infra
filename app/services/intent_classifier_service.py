"""Embedding-based k-NN intent classifier."""
...
def _initialize() -> None:
    ...
    with _init_lock:
        ...
        _example_vectors = _normalize_rows(np.array(vecs, dtype=np.float32))
...
def classify_by_embedding(
    ...
    Example and query embeddings are L2-normalized here so the dot product
    is equivalent to cosine similarity across embedding providers.
    """
    if not _initialized:
        _initialize()
